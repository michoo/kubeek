package com.kubeek.device.k3screendevice;

import com.kubeek.sdk.device.KScreenDevice;
import com.kubeek.utils.LibraryLoader;


public class K3Screen implements KScreenDevice {


    public K3Screen(int rows, int chain){
        this.init(rows,chain);
        System.out.println("K3Screen created rows: "+rows + " chain: "+chain);
    }

     static {
         try {
             LibraryLoader.loadLibrary("K3ScreenJNIDriver");
         } catch (Exception e) {
